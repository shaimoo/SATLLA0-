...
const unsigned long RX_DURATION = 15000; // 15 seconds
...
void loop()
{
  switch(state)
  {
    case TX:
      delay(1000);
      txNumber++;
        Serial.readStringUntil('\n').toCharArray(txpacket, sizeof(txpacket));
  ...
  while (true) {
            ...
            if (millis() - startTime >= RX_DURATION) {
...
void OnRxDone( uint8_t *payload, uint16_t size, int16_t rssi, int8_t snr )// resive image from the cubecell.
{
    Rssi=rssi;
    rxSize=size;
